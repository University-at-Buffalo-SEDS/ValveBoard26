...
def run_unacknowledged_can_simulation(
    ...
    for probe in layout["execution"]["memory_probes"]:
        ...
        if probe.get("name") == "umbilical_status_fail":
            # Depending on when the peripheral FIFO is sampled, the HAL can
            # either accept the frame or report the disconnected-link error.
            # Neither result is a liveness failure; forward TX progress and
            # the fatal probes below are the stable qualification contract.
            probe.pop("maximum", None)
            probe.pop("minimum", None)

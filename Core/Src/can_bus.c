...
static HAL_StatusTypeDef can_bus_recover_if_bus_off(void) {
  ...
  if (g_hfdcan == NULL || HAL_FDCAN_GetProtocolStatus(g_hfdcan, &status) != HAL_OK)
    return HAL_ERROR;
  if (status.BusOff == 0U)
    return HAL_OK;
  ...
  if (HAL_FDCAN_Stop(g_hfdcan) != HAL_OK || HAL_FDCAN_Start(g_hfdcan) != HAL_OK)
    return HAL_ERROR;
